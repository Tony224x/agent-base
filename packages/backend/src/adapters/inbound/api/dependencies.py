...
from src.adapters.outbound.auth.jwt_adapter import JWTAuthAdapter
from src.adapters.outbound.llm.openai_llm_adapter import create_llm
from src.adapters.outbound.persistence.memory_adapter import InMemoryPersistenceAdapter
from src.adapters.outbound.tools.mcp_adapter import MCPToolsAdapter
from src.core.domain.services.agent_service import AgentService
from src.core.domain.services.config_loader import load_agent_config
...
from src.core.use_cases.extract_client_info import ExtractClientInfoUseCase
...
def get_ask_question_use_case() -> AskQuestionUseCase:
    return AskQuestionUseCase(agent_service=get_agent_service())


def get_manage_conversations_use_case() -> ManageConversationsUseCase:
    return ManageConversationsUseCase(persistence=get_persistence_adapter())


def get_extract_client_info_use_case() -> ExtractClientInfoUseCase:
    return ExtractClientInfoUseCase(persistence=get_persistence_adapter())
